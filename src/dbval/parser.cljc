(ns ^:no-doc dbval.parser
  (:refer-clojure :exclude [distinct?])
  #?(:cljs (:require-macros [dbval.parser :refer [deftrecord]]))
  (:require
    [clojure.set :as set]
    [dbval.db :as db]
    [dbval.util :as util]))
...
#?(:clj
   (defmacro deftrecord
     "Augment all dbval.parser/ records with default implementation of ITraversable"
     [tagname fields & rest]
     (let [f    (gensym "f")
           pred (gensym "pred")
           acc  (gensym "acc")]
       `(defrecord ~tagname ~fields
          ITraversable
          (~'-postwalk [this# ~f]
            (let [new# (new ~tagname ~@(map #(list 'dbval.parser/postwalk % f) fields))]
              (if-let [meta# (meta this#)]
                (with-meta new# meta#)
                new#)))
          (~'-collect [_# ~pred ~acc]
            ;; [x y z] -> (collect pred z (collect pred y (collect pred x acc)))
            ~(reduce #(list 'dbval.parser/collect pred %2 %1) acc fields))
          (~'-collect-vars [_# ~acc]
            ;; [x y z] -> (collect-vars-acc (collect-vars-acc (collect-vars-acc acc x) y) z)
            ~(reduce #(list 'dbval.parser/collect-vars-acc %1 %2) acc fields))
          ~@rest))))
