(ns dbval.core
  ...
  (:require
    ...
    [dbval.conn :as conn]
    [dbval.db :as db #?@(:cljs [:refer [Datom DB FilteredDB]])]
    #?(:clj [dbval.pprint])
...
(defn ^DB empty-db
  "Creates an empty database with an optional schema.
   ...
   ```
   (empty-db) ; => #dbval/DB {:schema {}, :datoms []}

   (empty-db {:likes {:db/cardinality :db.cardinality/many}})
   ; => #dbval/DB {:schema {:likes {:db/cardinality :db.cardinality/many}}
   ;                    :datoms []}
   ```
...
(defn datoms
  "Index lookup. Returns a sequence of datoms (lazy iterator over actual DB index) which components (e, a, v) match passed arguments.
   ...
   Usage:
       ...
       (datoms db :eavt 1)
       ; => (#dbval/Datom [1 :friends 2]
       ;     #dbval/Datom [1 :likes \"fries\"]
       ;     #dbval/Datom [1 :likes \"pizza\"]
       ;     #dbval/Datom [1 :name \"Ivan\"])
       ...
       ; => (#dbval/Datom [1 :likes \"fries\"]
       ;     #dbval/Datom [1 :likes \"pizza\"]
       ;     #dbval/Datom [2 :likes \"candy\"]
       ;     #dbval/Datom [2 :likes \"pie\"]
       ;     #dbval/Datom [2 :likes \"pizza\"])
...
(def ^{:arglists '([conn tx-data] [conn tx-data tx-meta])} transact!
  ...
  Usage:
      ...
      (:tx-data report) ; => [#dbval/Datom [296 :name \"Ivan\"]]
...
(def ^{:doc "Data readers for EDN readers. In CLJS they’re registered automatically. In CLJ, if `data_readers.clj` do not work, you can always do
             ...
             ```"}
  data-readers {'dbval/Datom db/datom-from-reader
                'dbval/DB    db/db-from-reader})
