(ns ^:no-doc dbval.query
  (:require
    ...
    [dbval.pull-api :as dpa]
    [dbval.util :as util])
  #?(:clj
     (:import
       [clojure.lang ILookup LazilyPersistentVector]
       [dbval.parser BindColl BindIgnore BindScalar BindTuple
...
(defn parse-rules [rules]
  (let [rules (if (string? rules) (edn/read-string rules) rules)] ;; for dbval.js interop
