(ns ^:no-doc slateval.parser
  (:refer-clojure :exclude [distinct?])
  #?(:cljs (:require-macros [slateval.parser :refer [deftrecord]]))
  (:require
    [clojure.set :as set]
    [slateval.db :as db]
...
#?(:clj
   (defmacro deftrecord
     "Augment all slateval.parser/ records with default implementation of ITraversable"
     [tagname fields & rest]
     (let [f    (gensym "f")
           pred (gensym "pred")
           acc  (gensym "acc")]
       `(defrecord ~tagname ~fields
          ITraversable
          (~'-postwalk [this# ~f]
            (let [new# (new ~tagname ~@(map #(list 'slateval.parser/postwalk % f) fields))]
              (if-let [meta# (meta this#)]
                (with-meta new# meta#)
                new#)))
          (~'-collect [_# ~pred ~acc]
            ;; [x y z] -> (collect pred z (collect pred y (collect pred x acc)))
            ~(reduce #(list 'slateval.parser/collect pred %2 %1) acc fields))
          (~'-collect-vars [_# ~acc]
            ;; [x y z] -> (collect-vars-acc (collect-vars-acc (collect-vars-acc acc x) y) z)
            ~(reduce #(list 'slateval.parser/collect-vars-acc %1 %2) acc fields))
          ~@rest))))
