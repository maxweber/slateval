(ns ^:no-doc slateval.query
  (:require
    [clojure.edn :as edn]
    [clojure.set :as set]
    [clojure.string :as str]
    [clojure.walk :as walk]
    [slateval.built-ins :as built-ins]
    [slateval.db :as db]
    [slateval.arrays :as da]
    [slateval.lru :as lru]
    [slateval.impl.entity :as de]
    [slateval.parser :as dp ]
    [slateval.pull-api :as dpa]
    [slateval.util :as util])
  (:import
       [clojure.lang ILookup LazilyPersistentVector]
       [slateval.parser BindColl BindIgnore BindScalar BindTuple
...
(defn parse-rules [rules]
  (let [rules (if (string? rules) (edn/read-string rules) rules)] ;; for slateval.js interop
  ...
  (let [idx (attrs attr)]
    (if (contains? *lookup-attrs* attr)
      (if (int? idx)
        (let [idx (int idx)]
          (fn contained-int-getter-fn [tuple]
            (let [eid (if (.isArray (.getClass ^Object tuple))
      ...
      (if (int? idx)
        (let [idx (int idx)]
          (fn int-getter [tuple]
            (if (.isArray (.getClass ^Object tuple))
                      (aget ^objects tuple idx)
                      (nth tuple idx))))
        ;; If the index is not an int?, the target can never be an array
        (fn getter [tuple]
          (.valAt ^ILookup tuple idx))))))
