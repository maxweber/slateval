(ns slateval.core
  ...
  (:require
    ...
    [slateval.conn :as conn]
    [slateval.db :as db #?@(:cljs [:refer [Datom DB FilteredDB]])]
    #?(:clj [slateval.pprint])
    [slateval.pull-api :as dp]
...
(def ^{:tag Entity
       :arglists '([db eid])
       :doc "Retrieves an entity by its id from database. Entities are lazy map-like structures to navigate Dbval database content.
...
(defn ^DB empty-db
  "Creates an empty database with an optional schema.
   ...
   ```
   (empty-db) ; => #slateval/DB {:schema {}, :datoms []}

   (empty-db {:likes {:db/cardinality :db.cardinality/many}})
   ; => #slateval/DB {:schema {:likes {:db/cardinality :db.cardinality/many}}
   ;                    :datoms []}
   ```
...
(defn datoms
  "Index lookup. Returns a sequence of datoms (lazy iterator over actual DB index) which components (e, a, v) match passed arguments.
   ...
   Usage:
       ...
       (datoms db :eavt 1)
       ; => (#slateval/Datom [1 :friends 2]
       ;     #slateval/Datom [1 :likes \"fries\"]
       ;     #slateval/Datom [1 :likes \"pizza\"]
       ;     #slateval/Datom [1 :name \"Ivan\"])
       ...
       ; => (#slateval/Datom [1 :likes \"fries\"]
       ;     #slateval/Datom [1 :likes \"pizza\"]
       ;     #slateval/Datom [2 :likes \"candy\"]
       ;     #slateval/Datom [2 :likes \"pie\"]
       ;     #slateval/Datom [2 :likes \"pizza\"])
...
(def ^{:arglists '([conn])} conn?
  "Returns `true` if this is a connection to a Dbval db, `false` otherwise."
  conn/conn?)
...
(def ^{:arglists '([conn tx-data] [conn tx-data tx-meta])} transact!
  ...
  Usage:
      ...
      ; that will be replaced with Dbval to a next unused eid)
      (transact! conn [[:db/add -1 :name \"Ivan\"]])
      ...
      (:tx-data report) ; => [#slateval/Datom [296 :name \"Ivan\"]]
...
(def ^{:doc "Data readers for EDN readers. In CLJS they’re registered automatically. In CLJ, if `data_readers.clj` do not work, you can always do
             ...
             ```"}
  data-readers {'slateval/Datom db/datom-from-reader
                'slateval/DB    db/db-from-reader})
