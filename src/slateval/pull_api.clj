(ns ^:no-doc slateval.pull-api
  ...
  (:import
       [clojure.lang ISeq]
       [slateval.db Datom DB]
       [slateval.pull_parser PullAttr PullPattern]))

(declare pull-impl attrs-frame ref-frame ->ReverseAttrsFrame)

(defn- first-seq [^ISeq xs]
  (if (nil? xs)
    nil
    (.first xs)))

(defn- next-seq [^ISeq xs]
  (if (nil? xs)
    nil
    (.next xs)))

(defn- conj-seq [^ISeq xs x]
  (if (nil? xs)
    (list x)
    (.cons xs x)))
  ...
  (-run [this context]
    (loop [acc   acc
      ...
      (util/cond+
        ...
        (recur (assoc! acc (.-as attr) (.-default attr)) (first-seq attrs) (next-seq attrs))
