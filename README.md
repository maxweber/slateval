...
proof-of-concept that you can implement a library that offers Datomic-like
semantics on top of a mutable relational database like Sqlite.
...
already born in 2013 in a project where we were forced to use Postgres instead
of Datomic. It was a side quest ever since. We tried many different database
schemas. In 2025 a breakthrough were accomplished after learning about
...
Back to FoundationDB, there the keys and values are just byte arrays. The key
contains a tuple and its byte array representation allows to sort it, even if it
is a mix of different types (String, double, UUID, nested tuples, etc.). You
will notice that you can represent a (Datomic) datom as a tuple. Luckily, the
tuple to byte array logic is available via
https://apple.github.io/foundationdb/javadoc/com/apple/foundationdb/tuple/Tuple.html
...
split into many smaller transactions, to avoid that the transactor is not
occupied for too long. Otherwise customers have to wait a couple of seconds
before their write is processed. The next generation of our SaaS runs one
logical Datomic database per customer (with a shared transactor pair) to avoid
this issue. Your challenges may vary but I think it's important that open source
alternatives to Datomic exist, so that you have the option to make different
trade-offs.

Luckily there are already a couple of Datomic open source alternatives. Why another one? A couple of them does not offer you the database as a value. But the more important detail is the scope you pick. Implementing a database almost from scratch is a humongous task. For that reason dbval considers itself as a database library and only tries to marry Datascript with Sqlite. Similar like Clojure reaps more benefits by being hosted on the JVM instead of C for example.
